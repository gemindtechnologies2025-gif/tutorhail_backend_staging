const mongoose = require('mongoose');
const Schema = mongoose.Schema;
const bcrypt = require("bcryptjs");
const constants = require('../common/constants');

let tutorSchema = new Schema({
  name: {
    type: String,
    default: "",
    index: true
  },
  userName: {
    type: String,
    default: "",
    index: true
  },
  bioMetricId: {
    type: String,
    default: ""
  },
  email: {
    type: String,
    default: "",
    trim: true,
    lowercase: true,
    index: true
  },
  phoneNo: {
    type: String,
    default: "",
    index: true
  },
  dialCode: {
    type: String,
    default: "",
    index: true
  },
  countryISOCode: {
    type: String,
    default: ""
  },
  gender: {
    type: String,
    enum: Object.values(constants.GENDER)
  },
  age: {
    type: String,
    default: ""
  },
  image: {
    type: String,
    default: ""
  },
  tutorStatus: {
    type: Number,
    enum: Object.values(constants.TUTOR_STATUS),
    default: constants.TUTOR_STATUS.PENDING
  },
  isNotification: {
    type: Boolean,
    default: false
  },
  isPhoneVerified: {
    type: Boolean,
    default: false
  },
  isEmailVerified: {
    type: Boolean,
    default: false
  },
  isBlocked: {
    type: Boolean,
    default: false,
    index: true
  },
  isDeleted: {
    type: Boolean,
    default: false,
    index: true
  },
  isSocialLogin: {
    type: Boolean,
    default: false
  },
  appleId: {
    type: String,
    default: "",
    index: true
  },
  googleId: {
    type: String,
    default: "",
    index: true
  },
  linkedInId: {
    type: String,
    default: "",
    index: true
  },
  microsoftId: {
    type: String,
    default: "",
    index: true
  },
  jti: {
    type: String,
    default: "",
    select: false,
    index: true
  },
  deviceType: {
    type: String,
    enum: Object.values(constants.DEVICETYPE)
  },
  deviceToken: {
    type: String,
    default: "",
    index: true
  },
  loginCount: {
    type: Number,
    default: 0
  },
  password: {
    type: String,
    default: ""
  },
  isPasswordSet: {
    type: Boolean,
    default:false
  },
  shortBio: {
    type: String,
    default: ""
  },
  isProfileComplete: {
    type: Boolean,
    default:false
  },
  profileCompletedAt : {
    type: Number,
    enum: Object.values(constants.PROFILE_STATUS),
    default: constants.PROFILE_STATUS.DEFAULT
   },
  documentVerification: {
    type: Boolean,
    default : false
  },
  address: {
    type: String,
    default: ""
  },
  type: {
    type: Number,
    enum: Object.values(constants.CUSTOMER_TYPE),
    default: constants.CUSTOMER_TYPE.TUTOR
  },
  isActive: {
    type: Boolean,
    default: true
  },
  isOnline: {
    type: Boolean,
    default:false
  },
  longitude: {
    type: Number,
    default: 0
  },
  latitude: {
    type: Number,
    default: 0
  },
  location: {
    type: {
      type: String,
      default: "Point"
    },
    coordinates: {
      type: [Number],
      default: [0, 0]
    }
  }
},{
  timestamps: true
});

tutorSchema.methods.authenticate = function (password, callback) {
  const promise = new Promise((resolve, reject) => {
    if (!password) reject(new Error("MISSING_PASSWORD"));
    console.log("password", password);
    console.log("this password", this.password);
    bcrypt.compare(password, this.password, (error, result) => {
      if (!result) {
        reject(new Error("Invalid Password"));
      }
      resolve(this);
    });
  });

  if (typeof callback != "function") return promise;
  promise
    .then((result) => callback(null, result))
    .catch((err) => callback(err));
};

tutorSchema.methods.setPassword = async function (password) {
  console.log(password,"***************8");
  let hashPwd = await bcrypt.hash(password, 10);
  this.password = hashPwd;
  this.set({
    isPasswordSet : true
  });
  console.log(password);
  return this;
};

tutorSchema.index({
  location: '2dsphere'
});

tutorSchema.pre(/save|create|update/i, function (next) {
  if (this.get("latitude") && this.get("longitude")) {
    const longitude = this.get("longitude");
    const latitude = this.get("latitude");
    const location = {
      type: "Point",
      coordinates: [parseFloat(longitude), parseFloat(latitude)]
    };
    this.set({
      location
    });
  }
  next();
});

// const Tutor = mongoose.model('Tutor', tutorSchema);
// module.exports = Tutor;
module.exports = mongoose.model('Tutor', tutorSchema);